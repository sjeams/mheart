...
class Video extends ActiveRecord {
	...
	public static function getQueryUrl($page,$belong,$type=0,$search='')
    {
		...
		// 类型  0国产--1主播/  2少女  3熟女 4日韩  5其它  6AI/明星  7三级 8精品  9无码  10 收藏
			...
			if($belong==1){
				...
				$type = $type?$type:1;
				// tantanzy.com，tantanzy1.com
				//如果是图片
				if($category_id==2){
						$list=array($belong,$type,'探探',"/index.php/art/type/id/$type/page/$page.html",'http://www.tantanzy11.com');
				}else{
					if($search){
						// http://tantanzy11.com/index.php/vod/search/page/1/wd/邱淑贞.html
						$list=array($belong,$type,'探探',"/index.php/vod/search/page/$page/wd/$search.html",'http://www.tantanzy11.com');
						// array($belong,$type,'探探',"/type/2.html",'https://436727.166477.com'),
					}else{
						$list=	array($belong,$type,'探探',"/index.php/vod/type/id/$type/page/$page.html",'http://www.tantanzy11.com');
	...
	public static function videoDetailsMethod($data1,$type,$belong,$link,$isquery,$http){
		...
		//去标签
		$data1['title'] =strip_tags($data1['title']);

...
class UserBiologyNatureDo extends ActiveRecord
{
    ...
    public $action_move;//当前生物攻击\技能行动次数,默认为0
    public $history_count;//历史动作总条数
    ...
    function init(){
        ...
        $this->attack_bout=0;//生物序号。//根据前端索引排序0-9
        ...
        $this->biology_id=0;
        $this->action_move=0;//行动次数
        $this->history_count=0;//历史条数
    }
...
// ----------------------------------------
    ...
    public function fightingExtend($my_biology,$do_biology){
        ...
        //根据速度排序
        $merge_biology_list_su = array_column($merge_biology_list,'suDu');
        array_multisort($merge_biology_list_su,SORT_DESC,$merge_biology_list);
    ...
    public function getFightSystem($my_biology,$do_biology,$map_int=0){
        ...
        $biolgy_state =$UserWords->getBiolgyState();//返回境界
        //开启循环战斗--初始回合
        $this->fighting();//返回战斗记录
        $this->getFightingData();//重置索引0,处理history列表
        //返回战利品和战斗记录
        return [
    ...
    public function getFightingData(){
        ...
        foreach($fighting_history as$key => $vst){
            $list=[];
            // 循环合并二维数组
            foreach($vst as $vst_val){
                $list= array_merge_recursive($list,$vst_val);
            }
    ...
    public function getFighHurt(){
        ...
        foreach($this->merge_biology_extend as $attack_biology){
            ...
            if($attack_biology['shengMing']>0){
                ...
                // $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][0]['h_need']=[];//技能--消耗（耗蓝）
                // $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][0]['h_go']=[];//技能--增幅（被动
                // $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][0]['h_do']=[];//技能--主动（攻击）
                // $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][0]['h_putong']=[];//普通攻击
    ...
    public function attackPosition($attack_biology){
        ...
        if($fight_skill ){
            ...
            foreach($fight_skill as $skill){
                ...
                if($position_my==POSITION_MY){  //---判断是哪一方进攻
                    $position_my_list_new   = $this-> getShengMing(POSITION_MY);
                    $position_enemy_list_new= $this-> getShengMing(POSITION_ENEMY);
                }else{
                    $position_enemy_list_new   = $this-> getShengMing(POSITION_MY);
                    $position_my_list_new= $this-> getShengMing(POSITION_ENEMY);
                }
                //排序
                if($position_type){
                ...
                if($attack){ // 0 直接跳过--被攻击--被动跳过
                    ...
                    if(in_array($position_my,array(POSITION_MY,POSITION_ENEMY))){  
                        ...
                        $this->startHistory();  //声明数组类型
                        $attack_biology = $this->attackNeedValue($attack_biology,$skill);
                        //有pid行动才会写入动作  
                        //循环攻击位置  
                        foreach($int as $pid){
                            // is_do 0被动   1 主动 和主动触发
                            $this->attackSkill($position_my,$attack_biology,$pid,$skill,SKILL_GOING); //进行动作，并存入数据
                        }
                    } 
                    // var_dump($skill);//技能
        ...
        if(!$skill_attack){
            ...
            if($pid){
                //有pid行动才会写入动作  
                $this->startHistory();  //声明数组类型
                $this->attackPositionBlack($position_my,$attack_biology,$pid[0]); //进行动作，并存入数据
    ...
    //每次调用都会声明一次行动
    public function startHistory($action_move=null){
        //没有传固定行动次数，就默认累加
        if(!$action_move){
            $this->action_move++;
        }
        $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_yubei']=[];//预备回合-(计算)--眩晕--中毒--冰冻计算
        //发起顺序--根据情况选取普通攻击 putong   还是 技能攻击 do
        $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_need']=[];//技能--消耗（耗蓝）
        $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_go']=[];//技能--增幅（被动
        $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_putong']=[];//普通攻击
        $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_do']=[];//技能--主动（攻击）
        $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_back']=[];//技能 被动（反击）
        $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_end']=[];//回合结束-(解除)-眩晕--中毒--冰冻计算
    ...
    public  function attackSkillHurtValue($position_my,$doid,$pid,$attack_biology_go,$skill_go,$attack_biology_do,$is_do){
       ...
       $goid = $attack_biology_go['id'];
        ...
        $hurt_go_list = array(
            ...
            'not_out'=>1,//是否跳过  //0跳过 1不跳过--默认不跳过行动
            'is_baoji'=>0,//是否暴击
            'is_shanbi'=>0,//是否闪避
    ...
    public function goHurt($hurtType,$hurt_go_list){
        ...
        $not_out= $hurt_go_list['not_out']; //0跳过 1不跳过--默认不跳过行动
        //获取闪避
        ...
        if($attack==POSITION_ENEMY){
            ...
            $hurt_go = $hurt_go>=0?-1:$hurt_go;//伤害最少为-1
     
        }
        // $not_out=1;//0跳过 1不跳过--默认不跳过行动
        // if($is_do){ //0反击 1主动
            //主动---闪避直接跳过
        ...
        $hurt_go_list['is_baoji'] = $is_baoji;//是否暴击
    ...
    public function getTips($hurt_go_list){   
        ...
        $hurt_go_list['descript_go_msg'] ='';//消耗技能
        ...
        // var_dump($type.'-'.$is_do.'-'.$belong);

        // 0普通攻击  1技能攻击  2发起消耗 
        if($type==HURT_PUTONG){
            $hurt_go_list['descript_go'] =$sm_go.'普通攻击';
            if($shanbi){
                $hurt_go_list['extend'] ='shanbi'; //闪避时跳过动作
            ...
            $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_putong'][]= $hurt_go_list; //普通攻击
            ...
            if($is_do==SKILL_GOING){
                if($belong==ATTACK5){
                    $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_do'][]= $hurt_go_list; //技能--主动（攻击）
                }else{
                    $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_go'][]= $hurt_go_list; //技能--增幅（被动）
                }
            }else{
                $this->fighting_history[$this->bout]['fighting_history'][$this->attack_bout][$this->action_move]['h_back'][]= $hurt_go_list; //技能（反击）
            }

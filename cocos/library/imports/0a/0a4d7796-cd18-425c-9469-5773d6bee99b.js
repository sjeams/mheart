...
cc.Class({
  ...
  biology_buzhen_list: function biology_buzhen_list(TipBoxPrefab_model, TipBoxPrefab) {
    ...
    for (var prop in http_globalData.zhenfa) {
      ...
      var biology_id = http_globalData.zhenfa[prop]; //开始实例化预制资源

      var TipBoxPrefab_icon = cc.instantiate(http_globalData.model_home_zhenfa_biology); //写入详情

      this.biology_buzhen_detail(TipBoxPrefab_model, TipBoxPrefab, TipBoxPrefab_icon, biology_id); //拖拽逻辑

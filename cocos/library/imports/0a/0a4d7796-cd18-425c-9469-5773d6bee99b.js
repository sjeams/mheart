...
cc.Class({
  ...
  //技能图片渲染
  biology_buzhen_list: function biology_buzhen_list(TipBoxPrefab_model, TipBoxPrefab, biology, zhenfa) {
    ...
    cc.loader.loadRes('/model布阵/图标阵法', function (errorMessage, loadedResource_icon) {
      for (var prop in zhenfa) {
        var key = zhenfa[prop]; //开始实例化预制资源

        var TipBoxPrefab_icon = cc.instantiate(loadedResource_icon); //写入详情

        _this.biology_buzhen_detail(TipBoxPrefab_icon, biology, key); // 由于加载资源的操作是异步的，如果在加载完成前就绑定了事件，有可能会触发事件的自动执行。
        // 此处进行事件绑定
  ...
  biology_buzhen_detail: function biology_buzhen_detail(TipBoxPrefab_icon, biology, key) {
    if (key) {

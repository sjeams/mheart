...
var FightingExtend = cc.Class({
  ...
  playTips: function playTips(node, biology, is_skill) {
    ...
    if (biology.descript_go_msg != '') {
      ...
      node.getChildByName('技能s').runAction(cc.sequence(cc.scaleTo(0.5, 1.1, 1.1), cc.fadeOut(0.2), cc.scaleTo(0.01, 1, 1)), cc.callFunc(function () {
  ...
  playAction: function playAction(node, biology, is_skill) {
    ...
    if (is_skill != 0) {
      ...
      node.getChildByName('技能效果').opacity = 255; // node.getChildByName('技能效果').runAction(cc.fadeOut(1),cc.callFunc(function(){  node.getChildByName('技能效果').active=false },this));

      node.getChildByName('技能效果').runAction(cc.sequence(cc.scaleTo(0.5, 1.1, 1.1), cc.fadeOut(0.2), cc.scaleTo(0.01, 1, 1)), cc.callFunc(function () {
    ...
    if (biology.extend == 'moFa') {
      ...
      node.getChildByName('扣蓝s').getComponent(cc.Label).string = biology.hurt_msg; // node.getChildByName('魔法s').getComponent(cc.Label).string= biology.hurt_go_value+'/'+node.moFa
      ...
      node.getChildByName('扣蓝s').runAction(cc.sequence(cc.scaleTo(0.1, 1.1, 1.1), cc.moveBy(0.5, cc.v2(0, 60)), cc.fadeOut(0.2), cc.scaleTo(0.01, 1, 1), cc.moveBy(0.01, cc.v2(0, -60))), cc.callFunc(function () {
        node.getChildByName('扣蓝s').active = false;
      }, this)); // node.getChildByName('扣蓝s').runAction(cc.moveBy(0.2,cc.v2(0,60)));
      // node.getChildByName('扣蓝s').runAction(cc.fadeOut(1),cc.callFunc(function(){  node.getChildByName('扣蓝s').active=false },this));
    ...
    if (biology.extend == 'shengMing') {
      ...
      if (biology.hurt_go >= 0) {
        ...
        node.getChildByName('加血s').runAction(cc.sequence(cc.scaleTo(0.1, 1.1, 1.1), cc.moveBy(0.5, cc.v2(0, 60)), cc.fadeOut(0.2), cc.scaleTo(0.01, 1, 1), cc.moveBy(0.01, cc.v2(0, -60))), cc.callFunc(function () {
          node.getChildByName('加血s').active = false;
        }, this)); // node.getChildByName('加血s').runAction(cc.moveBy(0.2,cc.v2(0,60)));
        ...
        node.getChildByName('扣血s').runAction(cc.sequence(cc.scaleTo(0.1, 1.1, 1.1), cc.moveBy(0.5, cc.v2(0, 60)), cc.fadeOut(0.2), cc.scaleTo(0.01, 1, 1), cc.moveBy(0.01, cc.v2(0, -60))), cc.callFunc(function () {
          node.getChildByName('扣血s').active = false;
        }, this)); // node.getChildByName('扣血s').runAction(cc.moveBy(0.2,cc.v2(0,60)));

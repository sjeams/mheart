...
cc.Class({
  ...
  biology_detail_skill: function biology_detail_skill(TipBoxPrefab, position_skill) {
    ...
    for (var prop in position_skill) {
      ...
      cc.loader.loadRes('/弹窗模型/biology_生物_技能图标', function (errorMessage, loadedResource_icon) {
        ...
        TipBoxPrefab_icon = cc.instantiate(loadedResource_icon); //载入技能图片
        ...
        _this.bindClickEventIcon(TipBoxPrefab_icon.getComponent(cc.Button), skill); //写入icon
  ...
  // 绑定按钮事件
  bindClickEventIcon: function bindClickEventIcon(button, skill) {
    cc.log(111);
    cc.log(skill); // var clickEventHandler = new cc.Component.EventHandler();

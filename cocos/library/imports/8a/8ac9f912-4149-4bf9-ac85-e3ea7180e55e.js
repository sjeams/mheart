...
cc.Class({
  ...
  initInfo: function initInfo(info, biolgy_state, is_my) {
    ...
    this.node.getChildByName('生物等级s').getComponent(cc.Label).string = '等级' + info['grade'] + '(' + biolgy_state[info['state']] + ')'; //创建一个新button 并将其挂载到创建的精灵下
  ...
  onConfirBtn: function onConfirBtn(e, info) {
    var _this = this; // _this.biology_detail_alert(info)
    //技能图标挂载

...
cc.Class({
    ...
    var _loop = function _loop() {
      ...
      cc.loader.loadRes('/model背包/图标生物', function (errorMessage, loadedResource_icon) {
        ...
        TipBoxPrefab_icon.on('click', function () {
          ...
          TipBoxPrefab_model.getComponent('biology_infoTools').biology_detail_alert(TipBoxPrefab_model, info); // _this.bindClickEventIcon(TipBoxPrefab_icon.getComponent(cc.Button),info,TipBoxPrefab_model);
        }, this); //写入icon
  ...
  // 绑定按钮事件---挂载生物详情
  bindClickEventIcon: function bindClickEventIcon(button, info, TipBoxPrefab_model) {
    cc.log(111);
    cc.log(info); // TipBoxPrefab.getChildByName('缘分').on('click', function () {
    // 事件处理逻辑
    //移除挂载

    TipBoxPrefab_model.getChildByName('生物详情').removeAllChildren(); //重新挂载

    TipBoxPrefab_model.getComponent('biology_infoTools').biology_detail_alert(TipBoxPrefab_model, info); // }, this);
    //加载预制资源 PrefabUrl为 预制资源在 资源中的路径
    // 销毁所有弹窗--节点

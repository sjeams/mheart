...
cc.Class({
      ...
      if (isDragging) {
        ...
        if (http_globalData.is_zhenfa) {
          ...
          if (http_globalData.is_zhenfa_chaochu) {
            _this.biologyClickRemove(TipBoxPrefab); // TipBoxPrefab_model.getComponent('bag_zhenfaTools').biology_detail_tips(TipBoxPrefab_model,'操作成功!')
  ...
  movingInt: function movingInt(event, TipBoxPrefab_model, TipBoxPrefab) {
    ...
    TipBoxPrefab.getChildByName('阵法详情').children.forEach(function (targetNode) {
      ...
      if (rect.contains(point)) {
        ...
        _this.move_biologyClickRemove(TipBoxPrefab); //删除替换


        _this.biologyClickRemove(TipBoxPrefab); //删除移动
  ...
  // 移除布阵
  biologyClickRemove: function biologyClickRemove(TipBoxPrefab) {
  ...
  // 移除布阵
  move_biologyClickRemove: function move_biologyClickRemove(TipBoxPrefab) {
    //如果阵容已存在，就获取阵容id，并且移除

...
cc.Class({
  ...
  biology_detail_alert: function biology_detail_alert(TipBoxPrefab_model, goodsid, gooduse_type, button_name) {
    ...
    _this.biology_detail_list(TipBoxPrefab_model, TipBoxPrefab, info, gooduse_type, button_name); // 此处进行事件绑定


    _this.bind_button(TipBoxPrefab_model, TipBoxPrefab, info); //写入icon


    TipBoxPrefab_model.getChildByName('左边弹窗').addChild(TipBoxPrefab); // return TipBoxPrefab_model
    // })
    // return TipBoxPrefab_model
  },
  //list渲染
  biology_detail_list: function biology_detail_list(TipBoxPrefab_model, TipBoxPrefab, info_list, gooduse_type, button_name) {
    //获取点击物品的类型
    var _this = this;
    ...
    for (var prop in TOOLS) {
      ...
      if (info.gooduse == gooduse_type) {
        var num_height = num_height + 1; //检查资源加载
        // if( errorMessage ) { cc.log( '载入预制资源失败, 原因:' + errorMessage ); return; }
        // if( !(loadedResource_icon instanceof cc.Prefab ) ) { cc.log( '你载入的不是预制资源!' ); return; }
        // //开始实例化预制资源
        ...
        _this.bind_button_detail(TipBoxPrefab_model, TipBoxPrefab_icon, info, gooduse_type, button_name); // //写入icon
  ...
  //绑定点击事件--关闭遮罩
  bind_button: function bind_button(TipBoxPrefab_model, TipBoxPrefab_icon) {
  ...
  //绑定点击事件--alert详情
  bind_button_detail: function bind_button_detail(TipBoxPrefab_model, TipBoxPrefab_icon, info, gooduse_type, button_name) {
    TipBoxPrefab_icon.on('click', function () {
      TipBoxPrefab_model.getComponent('bag_详情Tools').biology_detail_alert(TipBoxPrefab_model, TipBoxPrefab_icon, info, gooduse_type, button_name, 1); //     // 事件处理逻辑
      //     //移除挂载
      //     TipBoxPrefab_model.getChildByName('左边弹窗').removeAllChildren();
      //     //重新挂载
      //     // TipBoxPrefab_model.getComponent('biology_skillTools').biology_detail_alert(TipBoxPrefab_model,info)
    }, this); // //查看详情--弹出一次已装备详情--如果点击带id，并且相等，弹窗窗口
